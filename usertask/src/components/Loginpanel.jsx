...
setData((prev)=>({...prev,[name]:value}))
  ...
  useEffect(()=>{
   ...
   if(!data.password.trim()&&focuspass.current===document.activeElement){
  ...
  const onSubmit=(e)=>{
    ...
    if(!regex.test(data.email)){
      valid.email='Enter valid email'
    }
    if(!passregex.test(data.password)){
      valid.password= 'Enter valid password'
    }
    if(Object.keys(valid).length>0){
      setError(valid)
    }
  ...
  return (
    <div className='w-full  h-screen flex justify-center items-center'>
      ...
      <div className='absolute top-[130px] pl-[20px] w-[50%] h-[250px] border-l-[1px] border-l-gray-600 right-0'>
        ...
        <label className="after:content-['*'] after:ml-0.5 after:text-red-500" htmlFor="">Email</label>
